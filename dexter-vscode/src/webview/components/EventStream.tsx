...
import type { AgentEvent, ToolStartEvent, ToolProgressEvent, ToolEndEvent, ToolErrorEvent } from '../types';
...
export function EventStream({ events }: EventStreamProps) {
  ...
  const latestThinking = useMemo(() => {
    ...
    return thinkingEvents.length > 0
  ...
  return (
    <div className="space-y-2">
      ...
      {!isDone && latestThinking && (
        <ThinkingIndicator message={latestThinking.message} />
      )}
      ...
      {events.filter((e) => e.type === 'context_cleared').map((event, i) => (
        <div key={`context-${i}`} className="p-2 bg-yellow-50 border border-yellow-300 rounded text-sm text-yellow-800">
          ⚠️ {event.message}
        </div>
      ))}

      {/* Tool limit message */}
      {events.filter((e) => e.type === 'tool_limit').map((event, i) => (
        <div key={`limit-${i}`} className="p-2 bg-orange-50 border border-orange-300 rounded text-sm text-orange-800">
          ⚠️ {event.message}
        </div>
      ))}
    </div>
